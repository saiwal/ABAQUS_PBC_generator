...
def pairnodes(msnds,slnds,coords,exclnds):
    ln1 = len(slnds)
    ln2 = len(msnds)
    if ln1<ln2:
        ln=ln1
    ...
    ci = 0
    # fill in coordinates from coords
    for i in msnds:
        cj = 0
        for j in coords:
...
#    pprint(coord_mas)
    ...
    # print(coord_pair)
    ci = 0
    # disp is the distance between master-slave surface
    disp = 1020
    for i in coord_mas:
        cj = 0
        for j in coord_mas:
        ...
        # print(coord_pair[ci])
        ci = ci + 1
    print(len(exclnds), len(coord_pair))
    c=0
    for a in coord_pair:
#        print(c)
        if (coord_pair[c][0] in exclnds) or (coord_pair[c][4] in exclnds):
            print("pop ",c)
            pprint(coord_pair[c])
            coord_pair.pop(c)
        else:
